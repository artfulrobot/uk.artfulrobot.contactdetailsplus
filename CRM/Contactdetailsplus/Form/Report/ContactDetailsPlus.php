 ...
 *
 */
class CRM_Contactdetailsplus_Form_Report_ContactDetailsPlus extends CRM_Report_Form {
        ...
        foreach ($table['fields'] as $fieldName => $field) {
          if (CRM_Utils_Array::value('required', $field) ||
            ...
            if ($table['alias'] == 'note_civireport') {
              ...
              $this->_columnHeadersComponent[$table['alias']]["{$tableName}_subject"]['title'] = 'Subject';
              $this->_columnHeadersComponent[$table['alias']]["{$tableName}_note"]['type'] = '';
              $this->_columnHeadersComponent[$table['alias']]["{$tableName}_note"]['title'] = 'Note';
              $this->_columnHeadersComponent[$table['alias']]["{$tableName}_contact_id"]['type'] = '';
              $this->_columnHeadersComponent[$table['alias']]["{$tableName}_contact_id"]['title'] = 'Note ContactId';
              continue;
            }
  ...
  /**
   * I think this fetches the component content per contact.
   *
   * e.g. fetches data for the relationships listing for all matching contacts.
   * It stores these values into a big array keyed by contact_id, then by
   * component.
   */
  function clauseComponent() {
    ...
    foreach ($this->_component as $component) {
      if (CRM_Utils_Array::value($component, $this->_selectComponent) && ($component != 'activity_civireport' && $component != 'relationship_civireport')) {
        $sql = "{$this->_selectComponent[$component]} {$this->_formComponent[$component]}
                         WHERE    {$this->_aliases['civicrm_contact']}.id IN ( $selectedContacts )
                         GROUP BY {$this->_aliases['civicrm_contact']}.id,{$component}.id ";

        $dao = CRM_Core_DAO::executeQuery($sql);
        // The component query is expected to have a contact id field named
        // civicrm_{component}_contact_id.
        $CC = 'civicrm_' . substr_replace($component, '', -11, 11) . '_contact_id';

        while ($dao->fetch()) {
          $countRecord = 0;
          $eligibleResult[$component] = $component;
          $row = array();
          foreach ($this->_columnHeadersComponent[$component] as $key => $value) {
            $countRecord++;
            $row[$key] = $dao->$key;
          }
    ...
    if (CRM_Utils_Array::value('relationship_civireport', $this->_selectComponent)) {
      ...
      $component = 'relationship_civireport';
      $eligibleResult[$component] = $component;
      $sql = "{$this->_selectComponent[$component]},{$this->_aliases['civicrm_contact']}.display_name as contact_b_name,  contact_a.id as contact_a_id , contact_a.display_name  as contact_a_name  {$this->_formComponent[$component]}
                         WHERE    ({$this->_aliases['civicrm_contact']}.id IN ( $selectedContacts )
                                  OR
      ...
      while ($dao->fetch()) {
        ...
        $rows[$dao->contact_a_id][$component][] = $row;
        ...
        $rows[$dao->civicrm_relationship_contact_id_b][$component][] = $row;
      }
    }
    ...
    if (CRM_Utils_Array::value('activity_civireport', $this->_selectComponent)) {
      ...
      $component = 'activity_civireport';
      $eligibleResult[$component] = $component;
      $sql = "{$this->_selectComponent[$component]} ,
                 {$this->_aliases['civicrm_activity_source']}.display_name as added_by {$this->_formComponent[$component]}
      ...
      while ($dao->fetch()) {
        ...
        if (isset($dao->civicrm_activity_source_contact_id)) {
          $rows[$dao->civicrm_activity_source_contact_id][$component][] = $row;
        }
        if (isset($dao->target_contact_id)) {
          $rows[$dao->target_contact_id][$component][] = $row;
        }
        if (isset($dao->assignee_contact_id)) {
          $rows[$dao->assignee_contact_id][$component][] = $row;
        }
      }

      //unset the component header if data is not present
      foreach ($this->_component as $component) {
        if (!in_array($component, $eligibleResult)) {

          unset($this->_columnHeadersComponent[$component]);
        }
      }
    }
  ...
  function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      if (!empty($row['civicrm_contact_image_URL'])) {
        $rows[$rowNum]['civicrm_contact_image_URL'] = "<img class=\"contact-report-plus__contact_photo\" src=\"{$rows[$rowNum]['civicrm_contact_image_URL']}\" alt=\"Contact photo\" />";
      }
